...
OCAD(네트워크 프로토콜 AI 이상 탐지 시스템)는 다양한 네트워크 프로토콜(BFD, BGP, PTP, CFM 등)에서 발생하는 성능 이상을 자동으로 탐지하고 분석하는 시스템입니다.
...
### 1.2 모니터링 대상 (CFM 프로토콜 예시)
> **참고**: 아래는 CFM 프로토콜의 예시입니다. 다른 프로토콜(BFD, PTP, BGP)은 각각의 특성에 맞는 장비와 메트릭을 모니터링합니다.
